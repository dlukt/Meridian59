 ...
 * Plain text messages are wrapped in JSON with timestamp.
 * Structured event payloads generated by the server are sent as raw JSON.
 * Uses round-robin across pipes for load distribution.
 * Non-blocking operation - returns false if no pipes are connected.
 * 
 * @param message The message content to send (plain text or structured JSON)
